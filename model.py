...
from tensorflow.keras.layers import Input, Conv2D, Add, UpSampling2D, MaxPooling2D, Lambda

from mrcnn import utils

# Requires TensorFlow 2.x and the latest Keras integrated with TensorFlow.
from distutils.version import LooseVersion
assert LooseVersion(tf.__version__) >= LooseVersion("2.0")

# No need to check Keras version separately, as it's included in TensorFlow 2.x
...
class MaskRCNN():
    ...
    def set_log_dir(self):
        """Sets the log directory for training."""
    ...
    def build(self, mode, config):
        ...
        if h % 64 != 0 or w % 64 != 0:
            raise Exception("Image size must be divisible by 64 to avoid issues "
        ...
        if mode == "training":
            ...
            gt_boxes = Lambda(lambda x: norm_boxes_graph(x, tf.shape(input_image)[1:3]))(input_gt_boxes)
        ...
        if mode == "training":
            ...
            anchors = Lambda(lambda x: anchors)(input_image)
        else:
        ...
        if mode == "training":
            ...
            if not config.USE_RPN_ROIS:
                input_rois = Input(shape=[config.POST_NMS_ROIS_TRAINING, 4], name="input_roi", dtype=tf.int32)
                target_rois = Lambda(lambda x: norm_boxes_graph(x, tf.shape(input_image)[1:3]))(input_rois)
            else:
            ...
            mrcnn_class_logits, mrcnn_class, mrcnn_bbox = fpn_classifier_graph(
                rois, mrcnn_feature_maps, input_image_meta, config.POOL_SIZE, config.NUM_CLASSES,
                train_bn=config.TRAIN_BN, fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE)

            mrcnn_mask = build_fpn_mask_graph(
                rois, mrcnn_feature_maps, input_image_meta, config.MASK_POOL_SIZE,
            ...
            model = Model(inputs=[input_image, input_image_meta, input_rpn_match, input_rpn_bbox,
                                input_gt_class_ids, input_gt_boxes, input_gt_masks],
                        outputs=[rpn_class_logits, rpn_class, rpn_bbox, rpn_rois,
                                mrcnn_class_logits, mrcnn_class, mrcnn_bbox, mrcnn_mask,
                                rpn_class_loss, rpn_bbox_loss, class_loss, bbox_loss, mask_loss])
        else:
            model = Model(inputs=[input_image, input_image_meta, input_anchors],
                        outputs=[rpn_class, rpn_bbox, rpn_rois,
    ...
    def find_last(self):
        ...
        if not dir_names:
            raise FileNotFoundError(f"No directories found in model directory: {self.model_dir}")

        key = self.config.NAME.lower()
        dir_names = filter(lambda f: f.startswith(key), dir_names)
        dir_names = sorted(dir_names)
        if not dir_names:
            raise FileNotFoundError(f"No model directories starting with {key} found in {self.model_dir}")
        ...
        if not os.path.exists(dir_name):
            raise FileNotFoundError(f"Selected directory does not exist: {dir_name}")
        
        # Find the last checkpoint
        checkpoints = next(os.walk(dir_name))[2]
        if not checkpoints:
            raise FileNotFoundError(f"No checkpoint files found in {dir_name}")
        
        checkpoints = filter(lambda f: f.startswith("mask_rcnn"), checkpoints)
        checkpoints = sorted(checkpoints)
        if not checkpoints:
            raise FileNotFoundError(f"No weight files found in {dir_name}")
    ...
    def load_weights(self, filepath, by_name=False, exclude=None):
        ...
        try:
            from tensorflow.keras.saving import saving
        except ImportError:
            # In very old versions, it might still use 'topology'.
            from tensorflow.keras.saving import saving as saving
    ...
    def compile(self, learning_rate, momentum):
        ...
        reg_losses = [
            regularizers.l2(self.config.WEIGHT_DECAY)(w)
